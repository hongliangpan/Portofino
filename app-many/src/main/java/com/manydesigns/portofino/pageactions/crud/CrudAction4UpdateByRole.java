...
@SupportsDetail
@PageActionName("Crud")
public class CrudAction4UpdateByRole<Object> extends CrudAction {
    ...
    @Buttons({
            @Button(list = "crud-read", key = "edit", order = 1, icon = Button.ICON_EDIT + Button.ICON_EDIT, group = "crud", type = Button.TYPE_SUCCESS),
    ...
    protected FormBuilder createFormBuilder() {
        ...
        try {
            ...
            for (PropertyAccessor accessor : cloneClassAccessor.getProperties()) {
                ...
                if (crudAcc != null && crudAcc.getCrudProperty() != null && crudAcc.getCrudProperty().isUpdatable()) {
    ...
    protected Resolution getEditView() {
        if(isPopup()) {
            return new ForwardResolution("/m/crud/popup/edit.jsp"+ getMenuLevel());
        } else {
