...
public class AbstractFormBuilder {
    ...
    protected void removeUnusedSelectionProviders(
        ...
        for (String[] current : selectionProviders.keySet()) {
            ...
            if (!isInProperties(propertyNames, currentNames)) {
    ...
    // Hongliangpan add 选择器的某个字段，在属性中【原来是所有属性propertyNames.containsAll(currentNames)】
    private boolean isInProperties(List<String> propertyNames, List<String> currentNames) {
        for (String currentName : currentNames) {
            if(propertyNames.contains(currentName)){
                return true;
            }
        }
        return false;
