...
public class FormBuilder extends AbstractFormBuilder {
    ...
    protected void buildField(com.manydesigns.elements.forms.FieldSet fieldSet,
        ...
        //hongliangpan add 通过 preCreate 配置是否可以编辑, preCreate优先级高于【SelectField默认根据注解，判断是否可以修改；】
        if (propertyAccessor instanceof CrudPropertyAccessor) {
            CrudProperty prop = ((CrudPropertyAccessor) propertyAccessor).getCrudProperty();
            field.setEnabled(prop.isEnabled());
            field.setInsertable(prop.isInsertable());
            field.setInsertable(prop.isUpdatable());
        }
        if (field != null) {
            fieldSet.add(field);
