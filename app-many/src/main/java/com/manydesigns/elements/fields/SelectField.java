...
public class SelectField extends AbstractField<Object> {
    ...
    public void readFromRequest(HttpServletRequest req) {
        ...
        //hongliangpan add
        if (DisplayMode.MultiDropdown == displayMode) {
            String[] stringValue = req.getParameterValues(inputName);
            if (stringValue == null) {
                return;
            }
            ...
            if (stringValue.length == 0) {
                ...
                value = OgnlUtils.convertValue(Joiner.on(",").join(stringValue), accessor.getType());
            }
    ...
    public void valueToXhtmlEdit(XhtmlBuffer xb) {
        switch (displayMode) {
            case MultiDropdown:
                valueToXhtmlEditMultiDropDown(xb);
                addCreateNewLink(xb);
                if (mode.isBulk()) {
                    xb.writeJavaScript(
                            "$(function() { " +
    ...
    public void valueToXhtmlEditMultiDropDown(XhtmlBuffer xb) {
        Object value = selectionModel.getValue(selectionModelIndex);
        Map<Object, SelectionModel.Option> options =
                selectionModel.getOptions(selectionModelIndex);
//TODO 分隔 ,连接的多个选项
        ...
        if (nextSelectField != null) {
            String js = composeDropMultiDownJs();
            xb.writeJavaScript(js);
        }
    }

    public String composeDropMultiDownJs() {
        StringBuilder sb = new StringBuilder();
        sb.append(MessageFormat.format(
                "$(''#{0}'').change(" +
                        "function() '{'" +
    ...
    public void valueToXhtmlEditAutocomplete(XhtmlBuffer xb) {
        ...
        xb.addAttribute("autocomplete", "on");
    ...
    public void valueToXhtmlView(XhtmlBuffer xb) {
        ...
        xb.addAttribute("id", id);
        //hongliangpan add name and value 选择器只读时,需要名称和value
        xb.addAttribute("name", id);
        xb.addAttribute("value", selectionModel.getValue(0).toString());
        if (href != null) {
            xb.openElement("a");
            xb.addAttribute("href", href);
        }
